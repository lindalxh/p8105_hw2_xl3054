...
questions:
1. P1 pols-month data: prez_gop = 2?
2. %>%  &  |>
3. can i use $ for column specific? NO
4. prob 2: omit rows that do not include dumpster-specific data, omit the total row?
          after combining rows, missing just leave as NA?
5. mr_trash_clean - date and year/month not match
6. suggested using piping, points off if we write one by one?
7. prob 3: merge based on county and date?
8. prob1 points?
...
library(tidyverse)
pols_month = read_csv(file = "./P1 data/pols-month.csv")
